...
| Разбор события (горячий путь, hot path) | `expected` | ошибка — норма: битая строка не редкость, а данность |
...
Ручных проверок нет: `and_then` продолжает успешный путь (happy path), `or_else` разбирает ошибку.
...
**Зачем это нужно:** некоторые операции STL **выбирают между move и copy** в зависимости от того, помечен ли move-конструктор (move constructor) `noexcept`. Если не помечен — STL копирует (потому что не может гарантировать strong guarantee при move).
...
- **`string_view` / `span`** — невладеющие view, для аргументов функций без копирования. Следите за временем жизни источника.
