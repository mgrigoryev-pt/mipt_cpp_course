...
- `end()` — итератор **за** последним (полуоткрытый диапазон, half-open range)
...
Алгоритмы STL требуют **минимальную** категорию. `sort` требует random access, поэтому для `list` есть отдельный `list::sort()`.
...
|---|---|---|
| `map<K,V>` | O(log n) | красно-чёрное дерево (red-black tree), **упорядочен** |
| `set<K>` | O(log n) | красно-чёрное дерево, **упорядочен** |
| `unordered_map<K,V>` | O(1)\* | хеш-таблица (hash table) |
| `unordered_set<K>` | O(1)\* | хеш-таблица |
...
`remove` **сдвигает** «выживших» в начало и возвращает итератор на новый логический конец. Хвост остаётся в состоянии valid but unspecified.
...
**Подвох:** `erase(it)` во время ручного обхода инвалидирует `it` (iterator invalidation), и `++it` читает мусор. `std::erase_if` (C++20) снимает проблему.
...
- **Итератор** — обобщённый указатель: `begin()`, `end()`, `++`, `*`, полуоткрытый диапазон `[begin, end)`. Категории: Input → Forward → Bidirectional → Random access → Contiguous, алгоритм требует минимальную.
