...
## 1.3 Перегрузка функций (overloading)
...
Синтаксически ссылка похожа на указатель, а по смыслу это синоним объекта.
...
3. Продление времени жизни временного (lifetime extension)
...
Move-семантика (move semantics) добавит пятый способ — передача rvalue для перемещения. Об этом в лекции 11.
...
## 3.6 Продление времени жизни временного

Обычно временный объект (temporary object, rvalue) живёт **до конца выражения**:
...
## 4.5 Функциональная запись каста

Каст можно записать и как вызов функции:
...
- Для **скалярных типов** `T(x)` — то же, что C-style каст. Всё та же боль.
- Для **классов** `T(x)` — это **вызов конструктора**, и это идиоматично.

Граница между «каст» и «вызов конструктора» здесь размыта — синтаксис общий.

---

## 4.6 Инициализация списком `T{x}`

C++11 добавил инициализацию списком (list-initialization) — она **запрещает сужающие преобразования** (narrowing):
