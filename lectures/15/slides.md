...
Копировать нельзя: два владельца → double free → UB.
...
Два указателя вместо одного. Копирование = два присваивания + **атомарный** (atomic) инкремент.
...
`weak_ptr` не увеличивает счётчик ссылок — он только наблюдает.
...
RAII здесь не спасает: каждая блокировка по отдельности корректна, проблема в **порядке** захвата.
...
- **Предикат обязателен** — защита от ложного пробуждения (spurious wakeup), когда поток просыпается сам
