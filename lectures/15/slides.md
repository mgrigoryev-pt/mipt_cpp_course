...
> Ресурс владеется объектом (ownership). Деструктор освобождает его автоматически — при любом выходе из области видимости (scope), включая исключение.

Тогда речь была только про **память**. В этой лекции видно, насколько идея универсальна: она работает для любого ресурса — памяти, потока, мьютекса (mutex).
...
![width:880px](img/unique-copy-vs-move.svg)

«Уникальное владение» = **move-only тип**. Это прямое применение правила пяти (rule of five) из лекции 11.
...
![width:900px](img/control-block.svg)

Инкремент **атомарный** (atomic) — копировать `shared_ptr` можно из разных потоков.
...
- **Атомарный счётчик** — чтобы копирование было потокобезопасным (thread-safe)
...
1. **Одна аллокация вместо двух** — объект и control block кладутся рядом. Быстрее, меньше фрагментации, лучше локальность (locality).
2. **Exception safety** — между `new T` и обёртыванием нет «дыры», где сырой указатель (raw pointer) мог утечь.

То же для `make_unique<T>(args)` (C++14). **Правило: голый `new` в умном указателе (smart pointer) — устаревший стиль.**
...
p.reset();          // деструктор вызван, память — нет
```

![width:880px](img/make-shared-layout.svg)
...
Ни один счётчик не дойдёт до нуля — объекты держат друг друга. **Утечка (memory leak), несмотря на умные указатели.**

Счётчик ссылок (reference count) — не сборщик мусора: он не умеет находить циклы.
...
![width:880px](img/refcount-cycle.svg)
...
Многопоточность стандартизирована с C++11. Поток — ресурс: перед уничтожением его нужно дождаться.
...
Плюс кооперативная отмена (cooperative cancellation):
...
Принудительно убить поток нельзя: остались бы захваченные мьютексы. **Правило: в новом коде `jthread`.**
...
Без мьютекса `++counter` — это read-modify-write, три операции. Два потока успевают вклиниться друг в друга, инкременты теряются. Формально — **гонка данных (data race), то есть UB**.
...
`lock_guard` — самая дешёвая и частая обёртка блокировки (lock). С C++17 работает CTAD, тип писать не нужно.

---

## Несколько мьютексов — новый класс ошибок

![width:900px](img/deadlock.svg)

RAII здесь не спасает: каждый `lock_guard` отработает честно, а программа всё равно встанет.
...
Внутри — алгоритм без deadlock (упорядочивание или `try_lock` с откатом). Даже если разные потоки передадут мьютексы в разном порядке, deadlock не случится.
...
| Память (много владельцев) | `shared_ptr` | `delete` при нулевом счётчике ссылок |
...
Одна идея — «владение выражается временем жизни (lifetime) объекта» — покрывает все эти случаи.
...
![width:880px](img/producer-consumer.svg)

Схема собирается из трёх частей: мьютекс защищает очередь, `condition_variable` будит, предикат проверяет условие.
...
`notify_one()` будит один ждущий поток, `notify_all()` — все.

---

## Что происходит внутри `wait`

![width:900px](img/cv-wait.svg)

Предикат обязателен: поток может проснуться и без `notify` — это ложное пробуждение (spurious wakeup), разрешённое стандартом.
...
Низкоуровневая форма: один поток обещает значение, другой ждёт. Полезно, когда результат приходит из колбэка (callback) или обработчика события. `set_exception` пробросит исключение.
...
- Любой доступ к shared mutable state — под мьютексом.
- Ожидание событий — через condition_variable.
- Асинхронность — через future/async.

Корректный — не значит быстрый: мьютекс упирается в системный вызов (system call). Ниже есть ещё один слой, и он за рамками курса:

- **`std::atomic<T>`** и **`std::memory_order`** — работа без мьютекса и модель памяти (memory model)
- **Lock-free** структуры данных, параллельные алгоритмы (`std::execution::par`)
- **Корутины** (coroutines, C++20) — обзорно
...
- `shared_ptr` — счётчик ссылок + control block, атомарные операции, дороже
...
- `std::scoped_lock` для нескольких мьютексов — без deadlock
...
- **Концепты** (concepts, C++20) — полностью
- **Ranges** — глубоко, реализация views
- **Аллокаторы** (allocators) — кастомные, `allocator_traits`
- **Move-семантика (move semantics):** ref-qualifiers, категории значения (value categories) целиком
- **Модель памяти C++:** атомарные операции, memory orderings, lock-free
- **`constexpr` deep:** `consteval`, `constinit`, виртуальные в compile-time
- **Бонусы:** выравнивание (alignment) / object model, observability
