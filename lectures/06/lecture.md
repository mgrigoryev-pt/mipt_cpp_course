...
Второе обычно неверно: грузовик не является двигателем, он его использует. Композиция гибче (можно заменить `engine_` в рантайме), не создаёт жёстких иерархий, не имеет проблем [множественного наследования](../glossary.md#multiple-inheritance) (multiple inheritance).

![Две панели. Слева: объект Dog с частью Animal внутри, стрелка в функцию void feed(Animal&) — подстановка разрешена. Справа: объект Car с полем Engine внутри, перечёркнутая стрелка в функцию void start(Engine&) — CE](img/is-a-has-a.svg)

Обратите внимание, чем эти два случая **не** различаются: в памяти картинка одна и та же — часть базы и поле лежат одинаково. Различается то, куда объект разрешено подставить.
...
![Две ленты байтов. Сверху объект A: одно поле, байты 0–3. Снизу объект B: та же A целиком по смещению 0 и поле B::b по смещению 4](img/layout-single.svg)
...
![Две ленты байтов. Сверху Event: kind в байте 0, семь байт padding, timestamp 8–15, pid 16–19, ещё четыре байта padding — 24 байта. Снизу CompactEvent: timestamp 0–7, pid 8–11, kind в байте 12, три байта padding — 16 байт](img/padding-order.svg)
...
![Две ленты байтов. Сверху раскладка, которой быть не может: база A в байтах 0–3 и double сразу за ней по смещению 4 — красным. Снизу настоящая: A, четыре байта padding, double по смещению 8](img/layout-padding.svg)
...
![Две ленты байтов. Сверху Holder: байт под поле tag, три байта padding, поле value — восемь байт. Снизу Tagged: сразу value с нулевого байта, пустая база не занимает места — четыре байта](img/empty-base.svg)

Каждый объект обязан иметь свой адрес: два разных объекта `Empty` не могут лежать по одному адресу, поэтому пустой класс занимает байт. А базой он может не занимать ничего: подобъект пустой базы разрешено разместить по тому же адресу, что и первое поле наследника. Это **[EBO](../glossary.md#ebo)** (empty base optimization; по-русски — оптимизация пустой базы), и её делают все основные компиляторы. На ней стоят типы стандартной библиотеки: `std::unique_ptr` со стандартным [deleter](../glossary.md#deleter) (лекция 2) занимает ровно столько, сколько указатель.
...
![Две панели. Слева: буфер байтов, int по смещению 1 пересекает границу машинного слова, помечен красным — UB. Справа: те же четыре байта копируются вызовом std::memcpy в переменную int pid, адрес которой выбрал компилятор](img/misaligned.svg)
...
![Две панели. Слева: объект b из частей A и B, розовая стрелка от A* pa на его начало — не копируется ничего. Справа: тот же объект, при копировании в переменную типа A переезжает только часть A, часть B перечёркнута красным](img/slicing.svg)
...
![Наследник B нарисован рамкой вокруг базы A. Слева номера конструкторов: A первый, B второй. Справа номера деструкторов: B первый, A второй. Внизу лента вызовов: A(), B(), ~B(), ~A()](img/ctor-order-chain.svg)
...
![Лента байтов объекта C: база A в байтах 0–3, база B в 4–7, поле C::c в 8–11. Розовые стрелки: A* pa указывает на нулевой байт, B* pb — на четвёртый, между ними подписан сдвиг в четыре байта](img/multiple-layout.svg)
...
![Слева граф наследования: Walker и Swimmer наследуют Animal, Duck наследует обоих. Справа объект Duck: в части Walker своя копия Animal::id, в части Swimmer вторая; красные стрелки от d.id ведут к обеим копиям — ambiguous, CE](img/diamond.svg)
...
![Слева тот же граф, но рёбра Walker и Swimmer к Animal розовые и помечены virtual. Справа объект Duck: части Walker и Swimmer, под ними одна общая Animal::id, к которой обе части обращаются через смещение](img/virtual-inheritance.svg)
