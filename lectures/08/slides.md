...
Каждый набор шаблонных параметров → отдельная функция в бинарнике. Это называется **инстанцирование** (instantiation).
...
1. **Первая фаза** — при чтении определения. Проверяется синтаксис, имена, не зависящие от шаблонных параметров.
2. **Вторая фаза** — при инстанцировании, когда компилятор знает `T`. Проверяются зависимые имена (dependent names).
...
f(42);          // 1: нешаблонная функция предпочтительнее
f(42.0);        // 2: T = double
int* p = nullptr;
...
Encoder<int,    int>           // основной шаблон (primary template)
```
...
Компилятор выбирает **наиболее специализированную** (partial ordering): чем уже паттерн, тем выше приоритет.
