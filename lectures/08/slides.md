...
Шаблоны — это **«инструкции компилятору, как породить код»**, не классы и функции сами по себе. Генерация функции под конкретный набор параметров называется **инстанцирование** (instantiation).

![width:900px](img/instantiation.svg)
...
Для **функций** компилятор выводит `T` из аргументов (template argument deduction). Для **классов** до C++17 приходилось писать явно:
...
С C++17 аргументы шаблона класса выводятся из **аргументов конструктора**:
...
Синтаксис: сигнатура (signature) конструктора `->` желаемый тип. Пользовательские guides — инструмент авторов библиотек; в прикладном коде почти не встречаются.

---
<!-- header: 5. Шаблоны псевдонимов и переменных -->

# 5. Шаблоны псевдонимов и переменных

**Шаблоны псевдонимов** (alias templates, C++11) — удобные сокращения:
...
Шаблон проверяется в две фазы — **двухфазный поиск имён** (two-phase lookup):

![width:900px](img/two-phase.svg)
...
Чтобы инстанцировать шаблон, компилятор должен видеть **его определение**, а не только объявление.

![width:860px](img/template-in-header.svg)

Отсюда же — рост времени компиляции в шаблонном коде.
...
Шаблонную функцию можно перегружать (overloading) обычной, шаблонной с другими параметрами и более специализированной:

![width:900px](img/overload-priority.svg)
...
**Полная специализация** (explicit specialization) — реализация для конкретных параметров:
...
## Какой паттерн сработает

Компилятор перебирает все специализации и выбирает из подошедших:

![width:900px](img/specialization-order.svg)

Если не подошла ни одна — работает **основной шаблон** (primary template). Если из подошедших ни одна не уже остальных — CE: неоднозначный вызов (ambiguous call).

---

## Как работает выбор

Такой порядок называется **partial ordering**: чем уже паттерн, тем выше приоритет.
...
`IsPointer<const int*>` подходит под **третью** специализацию — она наиболее специализированная. Через такие паттерны строятся `type_traits` и SFINAE — подробный разбор в курс не входит.
...
Третья строка **компилируется**, но это **не** частичная специализация (их для функций не существует), это **отдельный шаблон-перегрузка**. Работает по правилам разрешения перегрузки (overload resolution): более специализированный шаблон побеждает менее (см. раздел 7).

На практике для функций вместо «частичной специализации» используют именно перегрузку — она часто и предпочтительнее, потому что играет по тем же правилам.
...
- `enum` значения
- С C++20 — почти любой structural type: структуры с публичными полями
...
**Не** допускаются: `std::string` (не structural type), `float` до C++20, указатели на временные объекты (temporary objects).
...
В шаблонном коде часто возникает ситуация: имя зависит от `T` — это зависимое имя (dependent name), — и компилятор не знает, что это, тип или значение.
...
![width:900px](img/dependent-parse.svg)
...
Если у `T` нет `+=` — компилятор выдаст простыню на 100 строк из глубины инстанцирования («template error spew»). Лечат это концепты (concepts).
...
- **Двухфазный поиск имён:** ошибки часто всплывают в месте использования
- **Шаблонные функции, классы, шаблоны псевдонимов и переменных** — все они есть
...
- Variadic templates (пакеты параметров, parameter packs)
