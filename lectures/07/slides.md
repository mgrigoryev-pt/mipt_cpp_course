...
Без `virtual` функция выбирается **на этапе компиляции** по типу указателя (`Animal*`), а не объекта — **статическое связывание** (static binding). У виртуальной функции (virtual function) — иначе:
...
`virtual` включает **динамическое связывание** (dynamic binding): выбор функции по **реальному** типу объекта в рантайме. Это и есть **полиморфизм**.
...
`a.sound()` компилируется примерно в `(*a.vptr)[slot_of_sound](&a)` — те же три шага: **динамическая диспетчеризация** (dynamic dispatch).
