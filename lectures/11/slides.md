...
- **`std::move(x)`** превращает lvalue в rvalue. Само ничего не двигает — даёт компилятору **разрешение** забрать ресурс.
- **lvalue** имеют имя и адрес. **rvalue** — временные, которые сейчас умрут.
- **Move ctor / move=** принимают `T&&`, забирают ресурс, **зануляют** источник.
- **`noexcept`** на move-функциях — обязательно. Без него STL копирует вместо move.
- **Правило пяти:** написали один — подумайте про остальные. **Правило нуля:** обычно не пишите ничего, поля сами всё умеют.
...
За рамками курса про move остаётся:

- Universal references (`T&&` в шаблонах)
- `std::forward` и perfect forwarding
- Reference qualifiers методов
