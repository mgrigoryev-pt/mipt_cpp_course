...
// Не идеальной передачей — её курс не проходит, — а перемещением
// того, что фабрика уже держит по значению. Цена: один лишний move на
// создание правила, то есть перенос указателя. Выгода: фабрика начала
...
//
// ЗАНЯТИЕ 3.3. Аргументы по-прежнему берутся по значению — и это осознанно,
// а не потому, что лучше не умеем.
//
// Фабрика получает свою копию аргумента. Дальше у неё два варианта:
//
//   `args...`             ещё одна копия: сама переменная args — lvalue,
//                         и конструктор правила выберет копирующую форму;
//   `std::move(args)...`  перемещение: своя копия фабрике больше не нужна,
//                         она умрёт на закрывающей скобке.
//
// Второе и написано. Одна копия на создание правила вместо двух, и что
// важнее — фабрика начала принимать то, что копировать нельзя вообще
// (см. LambdaRule выше: предикат внутри Function move-only).
//
// Полностью убрать оставшуюся копию можно только идеальной передачей,
// а её курс не проходит — и правил создаётся десяток за весь
// запуск программы, так что убирать нечего. Горячие пути — event.h
// и agent.cpp; здесь холодно.
template <typename Rule, typename... Args>
std::unique_ptr<Rule> MakeRule(const std::string& id, Severity severity,
                               Args... args) {
    return std::make_unique<Rule>(id, severity, std::move(args)...);
}
