...
// в теле фабрики. Не «идеальной передачи» — её курс не проходит и первый
// курс до неё не доходит, — а именно перемещения того, что фабрика уже
// держит по значению. Пока правило создаётся напрямую, с std::move.
...
// безобидных процессов сложатся в одну сработку. Пустой ключ означает
// «считать всё вместе».
//
...
class ThresholdRule : public RuleBase {
 public:
    ...
    bool Check(const Event& event) override {
        ...
        // Буфер очищается после сработки: иначе одно превышение порога дало бы
        // сработку на каждом следующем событии, и на потоке это стало бы шумом.
        bucket->stamps.Clear();
        CountHit();
        return true;
    }
 ...
 private:
    ...
    Bucket* FindOrAddBucket(const std::string& key) {
        ...
        // Линейный поиск по ключам. Словарей ещё нет — они на занятии Ф1,
