// Тесты уборки на тике. Занятие Ф1.
//
...
TEST_CASE("счётчик заводится на каждый ключ") {
    ...
    // бы в одну сработку. Изъян в том, что до Ф1 эти счётчики никто не убирал.
...
TEST_CASE("нулевой простой отключает вытеснение счётчиков") {
    ...
    // set_idle_ms. Умолчание обязано ничего не менять — иначе на Ф1 сломались
...
TEST_CASE("тик снимает просроченные первые шаги") {
    ...
    // До Ф1 эти десять снимались только при следующем вызове Check —
