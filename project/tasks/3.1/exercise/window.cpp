// Примитив занятия 3.1: кольцевой буфер против односвязного списка.
//
// Сорок минут. Первая замена своего примитива своим же, более подходящим —
// и первый случай, когда «стало лучше» надо подтвердить числами, а не словами.
//
// Оба контейнера здесь свои, чтобы файл собирался сам по себе. Список —
// тот же, что писался на занятии 1.2: узел владеет следующим через unique_ptr,
// деструктор разбирает цепочку явным циклом.
//
// Порядок работы:
