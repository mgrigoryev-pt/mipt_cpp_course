// Примитив занятия 3.2: свой Function против std::function.
//
// Сорок минут. Двадцать пять строк стирания типа, после которых
// std::function перестаёт быть чёрным ящиком, — и замер, который показывает,
// что «своё короче» и «своё быстрее» это разные утверждения.
//
// Порядок работы:
//   1. Прочитать Function ниже и понять, где здесь виртуальный вызов
//      и где аллокация. И то, и то по одному, и оба находятся глазами.
//   2. Предсказать на бумаге: сколько аллокаций сделает каждый из двух
//      на маленькой лямбде и на большой. Числа не одинаковые.
...
//   cl /nologo /std:c++latest /O2 /W4 /EHsc function.cpp
//
...
// ---------------------------------------------------------------------------
// Свой Function — весь примитив занятия
// ---------------------------------------------------------------------------
//
// Задача: сложить в переменную одного типа любой вызываемый объект
// с подходящей подписью и вызвать его, ничего не зная о его типе.
//
// Приём тот же, что у ICondition на занятии 2.3: виртуальный вызов через
// базовый класс. Разница в том, что от вызываемого объекта НЕ требуется
// наследовать интерфейс — наследника делает за него шаблон. Поэтому лямбда,
// которая ничего наследовать не может, тоже подходит.

template <typename Signature>
class Function;  // объявлен, не определён: только специализация ниже
